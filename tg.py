...
4. Wait for <tool_response> before providing your final answer
4. NEVER fabricate data - always use tool results
5. After receiving <tool_response>, provide a natural language summary of the results
6. If a tool returns an error or empty results, acknowledge it honestly
7. **ONLY include arguments that the user EXPLICITLY mentioned** - do NOT guess or add fields the user didn't specify
8. All arguments are OPTIONAL - only include what the user actually asked for
9. **For nested objects and arrays**: Extract the data from the user's request and structure it according to the parameter schema. For example:
...
def contains_tool_call(response_text: str) -> bool:
    ...
    return "<tool_call>" in response_text or ('"tool_calls"' in response_text and '"name"' in response_text)
...
def execute_tool_call(tool_call: Dict[str, Any], tools: List[Dict[str, Any]], telegram_user_id: Optional[int] = None) -> Dict[str, Any]:
    """Execute a tool call and return the result."""
    logger.info(f"🔧 [execute_tool_call] Starting execution for tool: \"{tool_call['name']}\"")
    logger.info(f"📋 [execute_tool_call] Available tools: {[t['name'] for t in tools]}")
    if telegram_user_id is not None:
        logger.info(f"👤 [execute_tool_call] Including Telegram user ID: {telegram_user_id}")
    ...
    try:
        ...
        if contains_tool_call(response_text) and tools:
            ...
            if tool_calls:
                ...
                tool_result = execute_tool_call(tool_call, tools, telegram_user_id=user_id)
